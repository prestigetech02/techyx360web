...
export async function POST(request: Request) {
  ...
  try {
    ...
    const objectPath = `${folder}/${Date.now()}-${crypto.randomUUID().slice(0, 8)}.${extension}`
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: "Unable to upload avatar right now." },
        { status: 500 }
      )
    }
    ...
    console.error("Unexpected client avatar upload error", error)
    return NextResponse.json(
