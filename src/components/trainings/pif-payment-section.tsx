...
import { pifPaymentDetails } from "@/config/product-innovation-fellowship"
import { cn } from "@/lib/utils"

const labelClassName = "mb-2 block text-sm font-medium text-foreground"

type PifPaymentSectionProps = {
  disabled?: boolean
}

export function PifPaymentSection({ disabled = false }: PifPaymentSectionProps) {
    ...
    <div className="space-y-4 rounded-2xl border border-border/60 bg-muted/20 p-5 sm:p-6">
      <div>
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          {pifPaymentDetails.paymentNote}
        </p>
      </div>

      <div className="rounded-xl border border-brand/20 bg-brand/5 p-4">
        <p className="text-xs font-semibold tracking-wide text-brand uppercase">
          Pay {pifPaymentDetails.amount}
        </p>
        <dl className="mt-3 space-y-2 text-sm">
          <div className="flex flex-col gap-0.5 sm:flex-row sm:justify-between sm:gap-4">
            <dt className="text-muted-foreground">Bank</dt>
            <dd className="font-medium text-foreground">
              {pifPaymentDetails.bankName}
            </dd>
          </div>
          <div className="flex flex-col gap-0.5 sm:flex-row sm:justify-between sm:gap-4">
            <dt className="text-muted-foreground">Account number</dt>
            <dd className="font-medium text-foreground">
              {pifPaymentDetails.accountNumber}
            </dd>
          </div>
          <div className="flex flex-col gap-0.5 sm:flex-row sm:justify-between sm:gap-4">
            <dt className="text-muted-foreground">Account name</dt>
            <dd className="font-medium text-foreground">
              {pifPaymentDetails.accountName}
            </dd>
          </div>
        </dl>
      </div>
      ...
      <div>
        <label htmlFor="pif-paymentReceipt" className={labelClassName}>
