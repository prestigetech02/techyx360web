...
const TASK_SELECT =
  "id, title, notes, assignee_id, status, priority, scheduled_on, starts_on, ends_on, start_time, end_time, sort_order, created_by, created_at, updated_at"
...
function mapTaskRow(
  ...
  return {
    ...
    scheduledOn: row.starts_on ?? row.scheduled_on,
    startsOn: row.starts_on ?? row.scheduled_on,
    endsOn: row.ends_on ?? row.starts_on ?? row.scheduled_on,
    startTime: parseTimeOfDay(row.start_time),
    endTime: parseTimeOfDay(row.end_time),
    durationMinutes: minutesBetweenDateTimes(
      row.starts_on ?? row.scheduled_on,
      row.start_time,
      row.ends_on ?? row.starts_on ?? row.scheduled_on,
      row.end_time
    ),
